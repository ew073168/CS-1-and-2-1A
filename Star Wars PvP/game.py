...
while running == True:
	...
	if pygame.sprite.groupcollide(bullets1, shields, True, True, collided = None):
		p1shield = True
		p1shieldt = 5000
		start1 = pygame.time.get_ticks()

	if pygame.sprite.groupcollide(bullets2, shields, True, True, collided = None):
		p2shield = True
		p2shieldt = 5000
	...
	if p1shield == True:
		ploc = player.getcords()
		now = pygame.time.get_ticks()
		if now - start1 < p1shieldt:
			screen.blit(shield1, [ploc.x, ploc.y-16])
